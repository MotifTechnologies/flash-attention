...
def _flash_attn_fwd(
    ...
    if return_max_attn_logit:
        assert return_lse or requires_grad, (
            "return_max_attn_logit=True requires return_lse=True "
            "(kernel uses LSE's sScale path to communicate row_max to the correction warp)"
        )
    ...
    if return_max_attn_logit and arch // 10 == 9:
        raise NotImplementedError("return_max_attn_logit is only supported on SM100+ (Blackwell). SM90 (Hopper) is not yet implemented.")
    ...
    # The kernel writes per-row max logit (one value per query position per head).
    # Reduce to per-head max for the final output: max over all query rows.
